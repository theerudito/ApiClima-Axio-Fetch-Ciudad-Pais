  ...
  window.addEventListener("load", () => {
    ...
    if(navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(posicion  => {
          ...
          const tiempo = async () => {
            const api = "6a3b130f8cc2a2577fac82539b3ca3cb"
            let res = await axios.get(`http://api.openweathermap.org/data/2.5/weather?lang=es&units=metric&lat=${latitud}&lon=${longitud}&appid=${api}`)
              console.log(res);
